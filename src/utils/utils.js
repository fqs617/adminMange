...
export function enAndZn(val) {
  return ValidatorBase.pattern.enAndZn.test(val)
}

// 只能是汉字 不能超过10个汉字
export function vChineseTen(val) {
  return ValidatorBase.pattern.isChinese.test(val)
}
...
export function isPhoneNumber(phone) {
  if (!(ValidatorBase.pattern.isPhone.test(phone))) {
    return false
  }
  return true
}
...
// 判断图片格式
export function isTrueUpload (value) {
  let imgType = 'image/jpeg, image/png, image/jpg'
  if (value && imgType.indexOf(value) > -1) {
    return true
  }
  return false
}
// 检测包含汉字的字符长度
export function characterLen (val) {
